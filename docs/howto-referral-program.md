...
- self-referral (один человек создаёт второй аккаунт);
- мультиаккаунты и device farming;
...
1. Запрет self-referral (`referrerUserId !== referredUserId`).
...
$manager->revert($transactionIdToRollback, [
    ...
    'reason' => 'fraud_or_chargeback',
]);
```

## Практика по уровням лояльности + рефералка
...
- для новых аккаунтов можно включать усиленную проверку (большее окно pending).
...
flowchart TD
  ...
  C -- Ок --> D[Release в bonus_available]
